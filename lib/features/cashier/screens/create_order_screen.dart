...
class _CreateOrderScreenState extends State<CreateOrderScreen> {
  ...
  int _step = 1; // 1=pelanggan, 2=layanan, 3=bayar

  // Data
  Map<String, dynamic>? _selectedCustomer;
  List<Map<String, dynamic>> _services = [];
  List<Map<String, dynamic>> _cart = []; // {service, qty, subtotal}
  String? _voucherCode;
  Map<String, dynamic>? _voucherData;
  double _diskonVoucher = 0;

  // Pembayaran
  String _metodeBayar = 'cash'; // cash / non_cash
  ...
  Future<void> _searchCustomer(String query) async {
    ...
    try {
      final results = await _supabase
          ...
          .eq('role', 'customer')
          .or('nama_lengkap.ilike.%$query%,nomor_hp.ilike.%$query%')
          .limit(10);
      if (mounted) {
        setState(() {
    ...
    setState(() {
      if (idx >= 0) {
        ...
        _cart.add({
          'service': service,
          'qty': 1,
          'subtotal': (service['harga_per_satuan'] as num).toDouble(),
        });
      }
    });
  }
  ...
  Future<void> _simpanOrder() async {
    ...
    try {
      ...
      if (_voucherData != null) {
        await _supabase
            .from('reward_redemptions')
            .update({'status': 'digunakan', 'order_id': order['id']})
